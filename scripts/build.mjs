...
const WasmFileName = "wasm_template_bg.wasm";
...
async function buildTarget(target) {
  ...
  const cmd = `wasm-bindgen --weak-refs --target ${target} --out-dir ${target} ./target/wasm32-unknown-unknown/${profileDir}/wasm_template.wasm`;
  console.log(">", cmd);
  await $({
    cwd: WasmDir,
    stdout: "inherit",
  })`wasm-bindgen --weak-refs --target ${target} --out-dir ${target} ./target/wasm32-unknown-unknown/${profileDir}/wasm_template.wasm`;
}
